...
create policy "visitor creates profile" on public.chat_visitors for insert to authenticated with check ((select auth.uid()) = user_id);
...
create policy "visitor creates own conversation" on public.chat_conversations for insert to authenticated with check (exists (select 1 from public.chat_visitors v where v.id = visitor_id and v.user_id = (select auth.uid())));
create policy "visitor updates own conversation" on public.chat_conversations for update to authenticated using (exists (select 1 from public.chat_visitors v where v.id = visitor_id and v.user_id = (select auth.uid()))) with check (exists (select 1 from public.chat_visitors v where v.id = visitor_id and v.user_id = (select auth.uid())));
...
create policy "visitor writes own messages" on public.chat_messages for insert to authenticated with check (sender_type in ('visitor','system','ai') and exists (select 1 from public.chat_conversations c join public.chat_visitors v on v.id = c.visitor_id where c.id = conversation_id and v.user_id = (select auth.uid())));
...
create policy "agents write messages" on public.chat_messages for insert to authenticated with check (exists (select 1 from public.chat_agents a where a.user_id = (select auth.uid()) and a.role in ('admin','agent')));
