...
export async function GET(request: Request) {
  try {
    ...
    if (conversationId) {
      const { data, error } = await service.from("chat_messages").select("*").eq("conversation_id", conversationId).order("created_at", { ascending: true });
      if (error) throw error;
      return NextResponse.json({ messages: data ?? [] });
    }
    const { data, error } = await service.from("chat_conversations")
...
export async function POST(request: Request) {
  try {
    ...
    const { agent, service } = await requireAgent();
    ...
    if (body.action === "accept") {
      ...
      await service.from("chat_messages").insert({ conversation_id: body.conversationId, sender_type: "system", content: `${agent.name} from NextAura joined the conversation.` });
    ...
    if (body.action === "send") {
      ...
      const { data: conversation, error: conversationError } = await service.from("chat_conversations").select("status").eq("id", body.conversationId).single();
      if (conversationError) throw conversationError;
      if (conversation.status !== "human") return NextResponse.json({ error: "Accept the conversation before replying." }, { status: 409 });
      const { data, error } = await service.from("chat_messages")
        .insert({ conversation_id: body.conversationId, sender_type: "agent", sender_id: agent.id, content, metadata: { agent_name: agent.name } })
        .select("*").single();
      if (error) throw error;
      await service.from("chat_conversations").update({ last_message_at: now }).eq("id", body.conversationId);
    ...
    const { data, error } = await service.from("chat_conversations").update(update).eq("id", body.conversationId).select("*").single();
    if (error) throw error;
    await service.from("chat_messages").insert({ conversation_id: body.conversationId, sender_type: "system", content: body.action === "return_to_ai" ? "The conversation has been returned to the NextAura Assistant." : "This conversation has ended." });
    return NextResponse.json({ conversation: data });
  } catch (error) {
