...
export async function POST(request: Request) {
  try {
    ...
    if (body.conversationId) {
      const { data } = await service.from("chat_conversations").select("*").eq("id", body.conversationId).eq("visitor_id", visitor.id).maybeSingle();
      conversation = data;
    }
    if (!conversation) {
    ...
    const { data: visitorMessage, error: visitorMessageError } = await service.from("chat_messages")
    ...
    await service.from("chat_conversations").update({ language, lead_data: leadData, last_message_at: new Date().toISOString() }).eq("id", conversation.id);
    ...
    if (isHandoffRequest(content)) {
      ...
      const { data: systemMessage, error } = await service.from("chat_messages")
        .insert({ conversation_id: conversation.id, sender_type: "system", content: systemContent })
        .select("*").single();
      if (error) throw error;
      const { data: waitingConversation, error: waitingError } = await service.from("chat_conversations")
    ...
    const { data: history } = await service.from("chat_messages").select("*").eq("conversation_id", conversation.id).order("created_at", { ascending: true }).limit(16);
    const assistantContent = await generateAssistantReply({
      message: content,
      language,
      summary: conversation.summary,
      history: (history ?? []).map((message) => ({
        id: message.id, conversationId: message.conversation_id, senderType: message.sender_type,
        content: message.content, createdAt: message.created_at,
      })) as ChatMessage[],
    });
    const { data: assistantMessage, error: assistantError } = await service.from("chat_messages")
      .insert({ conversation_id: conversation.id, sender_type: "ai", content: assistantContent })
      .select("*").single();
    if (assistantError) throw assistantError;
    const { data: updatedConversation } = await service.from("chat_conversations").select("*").eq("id", conversation.id).single();
